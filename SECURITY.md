...
Imladris is intended for self-hosted use. It runs as a single MCP **stdio**
...
- **Secrets** are read from environment variables referenced by name
  (`api_key_env`) in config, stored in `~/.imladris/.env` (0600) and loaded at
  startup; they are redacted in logs and never returned by `imladris_status`.
  Never commit real config or `.env`.
- **Panel prompts leave your network** for any non-local provider. The `local-only`
  preset keeps every token on-prem; `imladris_status` reports which providers are
...
- **Council sessions persist history locally.** Session mode writes
  `~/.imladris/sessions/<thread_id>.json` (0600); the `thread_id` is charset-validated
  ...
  `imladris clear-sessions [thread_id]` (or the `imladris_clear_sessions` tool); the
...
- A recursion depth guard (`max_depth`) prevents an `imladris` call from
  re-entering itself when a panellist is itself an Imladris instance.
