...
def _roster_text(app: ImladrisApp) -> str:
...
def _action_menu(app: ImladrisApp) -> OptionList:
    return app.screen.query_one("#action-menu", OptionList)


def _action_labels(app: ImladrisApp) -> list[str]:
...
async def test_dashboard_renders_roster_roles_and_config_path(tmp_path, monkeypatch):
    ...
    async with app.run_test(size=(120, 36)) as pilot:
        ...
        assert "Judge = openai (gpt-4o)" in _dashboard_text(app, "#roles")
...
async def test_dashboard_refresh_catalog_action_reports_status(tmp_path, monkeypatch):
    ...
    def fake_refresh(**kwargs):  # noqa: ARG001
        return CatalogRefreshResult(
            ...
            status=CacheStatus(
                path=str(tmp_path / ".imladris/model-catalog-cache.json"),
                exists=True,
                updated_at=1,
                age_seconds=0,
                stale=False,
...
@pytest.mark.asyncio
async def test_dashboard_respects_imladris_config_and_default_missing_path(tmp_path, monkeypatch):
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("OPENAI_KEY", "secret")
    ...
    app = ImladrisApp(home=tmp_path, harness_status=_harness_status())
    async with app.run_test(size=(120, 36)) as pilot:
        await pilot.pause()
    ...
    missing_app = ImladrisApp(home=tmp_path, harness_status=_harness_status())
    async with missing_app.run_test(size=(80, 24)) as pilot:
        await pilot.pause()

        assert "~/.imladris/config.json" in _dashboard_text(missing_app, "#paths")
...
async def test_dashboard_malformed_json_surfaces_manual_repair_issue(tmp_path, monkeypatch):
    ...
    app = ImladrisApp(config_path=config_path, home=tmp_path, harness_status=_harness_status(False))
    async with app.run_test(size=(80, 24)) as pilot:
        await pilot.pause()
        issues = _dashboard_text(app, "#issues")
...
async def test_dashboard_branding_and_responsive_layout(tmp_path, monkeypatch):
    ...
    small = ImladrisApp(config_path=config_path, home=tmp_path, harness_status=_harness_status())
    ...
    roomy = ImladrisApp(config_path=config_path, home=tmp_path, harness_status=_harness_status())
    async with roomy.run_test(size=(120, 36)) as pilot:
        ...
        assert "imladris" in outer_title
...
def test_main_help_and_doctor_paths_remain_textual_free(tmp_path, monkeypatch, capsys):
    ...
    assert "Imladris configuration" in capsys.readouterr().out
