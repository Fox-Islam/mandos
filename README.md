...
which returns calibrated probabilities instead of prose.
...
judge stage produces measurements instead of assertions. Four consequences:
...
`probe` is the shape for a question that turns on a missing fact instead of on a
...
`hybrid`, `matrix` and `verify` each score every finding correctly and stably
...
your own model as the final author. The analysis schema is the same - consensus, contradictions, partial coverage, unique insights, blind spots - so the two
stay comparable.
...
found itself lacking, with how likely having it would change the answer. Instead of
guessing what to include up front, the calling model reads what the panel
...
orchestrator/        the service package
  ...
  judge/             the five shapes, their shared question sets, and the dispatcher
...
reasoning from probabilities instead of Jev's accuracy.
...
It exists because the support thresholds were measured, not chosen, and one of
