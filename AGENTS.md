...
The design and current contracts
...
- `orchestrator/` - the service package: `mcp_server.py` (FastMCP `mandos` +
  ...
  `noul`/`choice`/`score` primitives, provider switch, factory), `judge/` (the five
  shapes behind one dispatcher - `hybrid.py`, `matrix.py`, `verify.py`, `probe.py`,
  `llm.py`, plus `extract.py`, `answer_profile.py` and the shared `outcome.py` /
  `jev_common.py`), `sessions.py` (local session store +
  ...
  (JSON/YAML + env config; the `judge` block; roles panel/judge), `transcript.py` (the
  conversation a harness hook captured), `attribution.py` (the app name and URL sent
  to providers that record one), `http.py` (the pooled client every stage shares),
...
instead of building its own, and any process-level state it adds needs the same
...
probabilities instead of Jev's accuracy. When an assertion depends on the negatives,
...
and needs a key; it pins thresholds that were measured, not chosen, so change
